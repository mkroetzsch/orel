...
public class InstanceKBManager extends KBManager {
	...
	public void initialize() throws Exception {
		...
		storage.makePredicateAssertion("is",thing);
		storage.makePredicateAssertion("dinst",toptype,toptype);
		storage.makePredicateAssertion("dis",toptype);
		...
		for (int i=0; i<numerictypes.size(); i++) {
			...
			storage.makePredicateAssertion("dis",ntypeid);
		...
		for (int i=0; i<othertypes.size(); i++) {
			...
			storage.makePredicateAssertion("dis",otypeid1);
	...
	protected void registerPredicates() {
		...
		storage.registerPredicate( new PredicateDeclaration("is",1,true,false) ); // is(a): the element a necessarily exists
		storage.registerPredicate( new PredicateDeclaration("dis",1,true,false) );
		...
		storage.registerPredicate( new PredicateDeclaration("sdupsome",3,false,false) ); // dsupsome(A,P,D): A ⊑ ∃P.D
