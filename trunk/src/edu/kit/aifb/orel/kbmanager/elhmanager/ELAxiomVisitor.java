...
public class ELAxiomVisitor implements OWLAxiomVisitorEx<Boolean> {
	...
	public ELAxiomVisitor(StorageDriver sd, OWLDataFactory datafactory, BasicExpressionVisitor expvisitor, int todos) {
		storage = sd;
		this.todos = todos;
		...
		for (int i=0; i<descs.length; i++) {
			...
			for (int j=i+1; j<descs.length; j++) {
				...
				if ( (todos & ELKBLoader.PREPARE) != 0 ) {
					...
					if ((todos & ELKBLoader.PREPARECHECK)!=0) {
	...
	public Boolean visit(OWLDisjointDataPropertiesAxiom axiom) {
		...
		for (int i=0;i<props.length;i++) {
			...
			if ( (todos & (ELKBLoader.ASSERT | ELKBLoader.CHECK)) != 0 ) {
